...
class SwingCal extends JFrame {
	...
	public SwingCal(ReserveWrite r) {
		reserve = r;
	...
	public void calSet() {
		...
		for (int i = cal.getMinimum(Calendar.DAY_OF_MONTH); i <= cal
			...
			if (memoday == 1) {
				...
				if (SwingCal.this.reserve.target  == SwingCal.this.reserve.cal4) {	
				String currSDate = SwingCal.this.reserve.startT2.getText().trim();
				...
				//   비포달이    <   현재달  ||  비포년 <= 현재년
				if(target1  ==  btnAfter){
					System.out.println("에프터 진입 : "+ this.year + "    "+ year2);
					System.out.println("에프터 진입 : "+ this.month + "    "+ month2);
					/*if(this.year > year2 && this.month < month2) {
	...
	class ButtonEvent implements ActionListener {
		public void actionPerformed(ActionEvent ae) {
			...
			if (ae.getSource() == btnBefore) {
				...
				if (SwingCal.this.reserve.target == SwingCal.this.reserve.cal3) {
					SwingCal.this.reserve.startT2.setText(calDate);
				} else if (SwingCal.this.reserve.target == SwingCal.this.reserve.cal4) {

					

					String currSDate = SwingCal.this.reserve.startT2.getText();
...
/*					System.out.println("년 :" + chkYear);
					...
					SwingCal.this.reserve.stopT2.setText(calDate);
				}
...
public class Calender {
	...
	public Calender(ReserveWrite r) {
		SwingCal a = new SwingCal(r);
